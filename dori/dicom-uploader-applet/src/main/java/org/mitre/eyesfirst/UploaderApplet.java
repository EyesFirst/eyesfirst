...
public class UploaderApplet extends Applet {
	...
	private class ProcessDeIDRunnable implements Runnable {
		...
		public void run() {
			try {
				...
				List<DicomObject> dicomDump = uploader.process(dicomFile, imageryFile, efid, new JSObjectProgressMonitor(callbacks));
	...
	public void init() {
		...
		preferences = Preferences.userRoot().node("org/mitre/eyesfirst/uploader");
		currentDirectory = preferences.get("currentDirectory", null);
		if (currentDirectory != null)
			fileChooser.setCurrentDirectory(new File(currentDirectory));

		Uploader.cookieParam = getParameter("Cookie");
		Uploader.HOSTNAME = getParameter("hostname");

		try {
			System.setProperty("org.dcm4che2.imageio.ImageReaderFactory",
					"etc/ImageReaderFactory.properties");
			System.setProperty("org.dcm4che2.imageio.ImageWriterFactory",
					"etc/ImageWriterFactory.properties");
		} catch (Exception e) {
			e.printStackTrace();
		}
