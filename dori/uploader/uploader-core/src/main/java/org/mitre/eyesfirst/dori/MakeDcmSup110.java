...
public class MakeDcmSup110 {
	...
	public static void updateDicomObject(Uploader uploader, Zeiss2Dcm zeiss2Dcm, DicomObject nds) {
		...
		String oldSeriesUid = uploader.getOldSeriesUID(nds
				.getString(Tag.SeriesInstanceUID));
		String seriesUid;
		if (uploader.hasOldSeriesUID(oldSeriesUid)) {
			System.out.println("Series UID found");
			seriesUid = uploader.getOldSeriesUID(oldSeriesUid);
		} else {
			seriesUid = makeUid(nds);
			uploader.putOldSeriesUID(oldSeriesUid, seriesUid);
		}

		String oldStudyUid = uploader.getOldStudyUID(nds
				.getString(Tag.StudyInstanceUID));
		String studyUid;
		if (uploader.hasOldStudyUID(oldStudyUid)) {
			System.out.println("Study UID found");
			studyUid = uploader.getOldStudyUID(oldStudyUid);
		} else {
			studyUid = makeUid(nds);
			uploader.putOldStudyUID(oldStudyUid, studyUid);
		}
