...
function [ft_fundus,st_fundus] = OCTenface2FundusTensorForm(ft_oct,st_oct,interpCoordFlag,CCPeak,dimFundus,dimOCT,dimInterpOCT,scFactor) 
...
% else
%     ft_oct = reshape(ft_oct,1,aa);
%     st_oct = reshape(st_oct,1,aa);
%     CCPeak = reshape(CCPeak,2,1);
% end;
if interpCoordFlag == 1 % coordinates are for interpolated image
    % fundusInd = round(repmat(CCPeak,1,aa)+TRmat*[ft_oct;st_oct]); % note that the order of the components is (fast-time (jj),slow-time(ii))
    ft_fundus = round(CCPeak(1)+scFactor(1)*ft_oct);
    ft_fundus = min(ft_fundus,dimFundus(2));
    ft_fundus = max(ft_fundus,1);
    st_fundus = round(CCPeak(2)+scFactor(2)*st_oct);
    st_fundus = min(st_fundus,dimFundus(1));
    st_fundus = max(st_fundus,1);
    
%     ft_fundus = fundusInd(1,:);
%     st_fundus = fundusInd(2,:);
%     I2 = find(ft_fundus > dimFundus(2));
%     if ~isempty(I2)
%        ft_fundus(I2) = NaN;
%       % st_fundus(I2)  = NaN;
%     end;
%     I2 = find(st_fundus > dimFundus(1));
%     if ~isempty(I2)
%        st_fundus(I2) = NaN;
%       % st_fundus(I2)  = NaN;
%     end;
   ...
   % fundusInd = round(repmat(CCPeak,1,aa)+TRmat*[ft_oct_interp;st_oct_interp]); % note that the order of the components is (fast-time (jj),slow-time(ii))
%     ft_fundus = fundusInd(1,:);
%     st_fundus = fundusInd(2,:);
    ft_fundus = round(CCPeak(1)+scFactor(1)*ft_oct_interp);
    st_fundus = round(CCPeak(2)+scFactor(2)*st_oct_interp);
  %  I2 = find(ft_fundus > dimFundus(2));
    ft_fundus = min(ft_fundus,dimFundus(2));
    ft_fundus = max(ft_fundus,1);
%     if ~isempty(I2)
