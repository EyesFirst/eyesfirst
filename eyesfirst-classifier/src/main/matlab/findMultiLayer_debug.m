...
function [csRelTop,csRelIm,BdryRelTop,BdryRelIm,wcfim] = findMultiLayer_debug(cf,imfloor,imtop,mas,maxRangeCf,Nlayers,maxInterLayerDist,minInterLayerDist,ofile);
...
wcfim = NaN*ones(aa,bb,Nlayers);
cfim = NaN*ones(aa,bb,Nlayers);
cfimM1 = NaN*ones(aa,bb,Nlayers);
for ii = 1:bb
    for jj = 1:Nlayers
...
for ii = 1:bb
    for jj = 1:Nlayers
        ...
        if imfloor2(ii,jj) < imfloor(ii)
          ...
          floorCost = cf(imfloor(ii),ii);
          ccfmf = ccf-(floorCost+1); % set floor to a value of -1;
          wcf = [ccfmf(1:end-1);0]-[ccfmf(2:end);1];
        end;
        wcfim(imtop2(ii,jj):imfloor2(ii,jj),ii,jj) = wcf;
        cfimM1(imtop2(ii,jj):imfloor2(ii,jj),ii,jj) = ccfmf;
        cfim(imtop2(ii,jj):imfloor2(ii,jj),ii,jj) = ccf;
        wf(fi:li) = wcf;
        NpixCur = li-fi+1;
        locPixInd = 1:NpixCur;
        % downward edges
