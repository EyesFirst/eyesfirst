...
function [polyGate,SAATB] = initialLayerId(SAA,ofile,edf,ndil,sliceInd)
[aa,bb,cc] = size(SAA);
maxNumLayers = 10;
minmax = 1.0e-4;
minLowerGateVal = 24;
...
%         end;
        ...
        if max(max(curvec2)) > minmax && max(max(curvec1)) > minmax 
           ...
           gate = extractGate(ppg,plotFlag0,[]);
...
%         end;
    ...
    % imposeBound2 = 1;
    [curTB,curPolyGate] = initialTopBottom_nf(curim,curGate,imposeBound);
    SAATB(:,:,ii) = curTB;
