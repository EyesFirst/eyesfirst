...
public class CachingDicomAccessService implements DicomAccessService {
	// TODO: Refactor this to use BasicCache
	...
	private static class CacheEntry {
		...
		private final Logger log;
		private SoftReference<DicomImage> entry;
		 ...
		 */
		public synchronized DicomImage get(DicomAccessService service) throws DicomAccessException {
			if (entry != null) {
				DicomImage result = entry.get();
				if (result != null) {
			...
			DicomImage image = service.retrieveDicomObject(id.getStudyUID(), id.getSeriesUID(), id.getObjectUID());
			if (image == null) {
				...
				// The size varies, but 10MB is (sadly) a good guess
				entry = new SoftReference<DicomImage>(image);
				return image;
			}
		}
	}
