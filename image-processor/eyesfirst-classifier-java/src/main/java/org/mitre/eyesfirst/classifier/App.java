...
public class App {
	private static Options createOptions() {
		...
		/*Option fundus = new Option("f", "fundus", true, "(optional) a URL to pull a fundus photo from for blood vessel analysis");
		fundus.setArgName("URL");
		options.addOption(fundus);*/
		...
		Option key = new Option("k", "key", true, "(optional) a value to be returned in the callback POST as the \"key\" field");
		key.setArgName("KEY");
	...
	public static void main(String[] args) {
		...
		try {
			...
			EyesFirstClassifier classifier = new EyesFirstClassifier(wadoURI, destDir, cl.getOptionValue("dicom"), callbackURI, cl.getOptionValue("key"));
			//classifier.setFundusURI(cl.getOptionValue("fundus"));
			classifier.runAll();
			...
			// Also catch errors (don't do this normally) to make SURE we call
			// exit.
			System.err.println("Unable to run thickness classifier:");
			e.printStackTrace();
			System.exit(1);
			// Rethrow it anyway since we can't really handle them. Of course,
			// this probably will never be hit anyway.
			throw e;
		}
	}
}
