...
import org.mitre.eyesfirst.retinalthickness.classifier.RetinalThickessClassifier;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import com.mathworks.toolbox.javabuilder.MWException;

public class ThicknessClassifier implements Runnable {
	...
	public ThicknessClassifier(URI wadoURI, File destDir, String dicomURI, URI callbackURI) throws URISyntaxException {
		this.wadoURI = wadoURI;
		if (!this.wadoURI.getScheme().equals("http")) {
			throw new RuntimeException("Thickness classifier only supports HTTP URLs.");
		}
	...
	private void runDownload() throws ClientProtocolException, IOException {
		// Create the file to download it into
		FileOutputStream out = new FileOutputStream(new File(destDir, "original.dcm"));
		...
		if (headers.length > 0) {
			...
			if (!headers[headers.length-1].getValue().startsWith("application/")) {
				throw new IOException("Bad content type " + headers[headers.length-1].getValue());
			}
		}
		HttpEntity entity = response.getEntity();
		long length = entity.getContentLength();
		log.info("Downloading {} bytes...", (length > 0 ? Long.toString(length) : "an unknown number of"));
	...
	private void runMatlabProcess() throws MWException, ProcessorException {
		...
		// Matlab context.
		RetinalThickessClassifier tc = new RetinalThickessClassifier();
		tc.call_thickness_classifier(destDir.getAbsolutePath());
	...
	private void runCallback() throws IOException {
		...
		try {
			...
			String qs = wadoURI.getRawQuery();
			if (qs != null) {
				int i = qs.indexOf("studyUID=");
				if (i > 0) {
					qs = qs.substring(i);
				}
				entity.addPart("rawQueryString", new StringBody(qs, "text/plain", utf8));
			}
			String dicomResultURI = findDicomResultURI();
			if (dicomResultURI != null) {
				entity.addPart("processedQueryString", new StringBody(dicomResultURI, "text/plain", utf8));
			}
		...
		entity.addPart("thicknessMap", new FileBody(getThicknessMap()));
		entity.addPart("fundusPhoto", new FileBody(getSynthesizedFundus()));
	...
	private String findDicomResultURI() throws IOException {
		...
		return createDicomResultURI(metadata);
	}

	private static Logger logger() {
		return LoggerFactory.getLogger(ThicknessClassifier.class);
	}
	...
	public void runAll() throws IOException, MWException, Exception {
		...
		runDownload();
		System.out.println("STATUS:{\"message\":\"Starting MATLAB...\"}");
		runMatlabProcess();
	...
	/**
	 * Runs the entire thickness classifier process. Since this implements
